...
def run(i):
    output_str = subprocess.run(f'powershell cat in/{i:04}.txt | .\\target\\debug\\ahc049.exe > out/{i:04}.txt', shell=True, capture_output=True, text=True).stderr
...
if __name__ == '__main__':
    ...
    print()
    # '''
    df = pd.DataFrame(result, columns=['i', 'n', 'score', 'time'])
